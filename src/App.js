...
const defaultGrowthData = {
  "2026-03": { IG: 1200, TIKTOK: 850, XHS: 360, notes: "本月粉絲總數紀錄。" },
};
...
function getEntriesFromDay(dayValue) {
  if (!dayValue) return [];
  if (Array.isArray(dayValue)) return dayValue.map(normalizeEntry);
  if (typeof dayValue === "object" && Array.isArray(dayValue.items))
    return dayValue.items.map(normalizeEntry);
  if (typeof dayValue === "object") return [normalizeEntry(dayValue)];
  return [];
}
function packEntriesForSave(entries) {
  const valid = entries
    .map(normalizeEntry)
    .filter(
      (e) =>
...
function getDaySummary(entries) {
  return {
    total: entries.length,
    hasPost: entries.some((e) => e.hasPost),
    hasVideo: entries.some((e) => e.hasVideo),
    hasStory: entries.some((e) => e.hasStory),
    scheduled: entries.some((e) => e.scheduled),
    published: entries.some((e) => e.published),
  };
}
function getPlatformMeta(platform) {
  return PLATFORM_OPTIONS.find((p) => p.value === platform) || null;
}
function getClientId() {
  try {
    const e = localStorage.getItem(CLIENT_ID_KEY);
    if (e) return e;
    const id = `client_${Math.random().toString(36).slice(2)}_${Date.now()}`;
...
function normalizeGrowthRow(raw) {
  if (!raw) return { IG: 0, TIKTOK: 0, XHS: 0, notes: "" };
  if (typeof raw.IG === "number" || typeof raw.IG === "string")
    return {
      IG: Number(raw.IG || 0),
      TIKTOK: Number(raw.TIKTOK || 0),
      XHS: Number(raw.XHS || 0),
      notes: raw.notes || "",
    };
  if ("IG_base" in raw || "IG_delta" in raw)
    return {
      IG: Number(raw.IG_base || 0) + Number(raw.IG_delta || 0),
      TIKTOK: Number(raw.TIKTOK_base || 0) + Number(raw.TIKTOK_delta || 0),
      XHS: Number(raw.XHS_base || 0) + Number(raw.XHS_delta || 0),
      notes: raw.notes || "",
    };
  return { IG: 0, TIKTOK: 0, XHS: 0, notes: "" };
}
function computeGrowthRows(growthData) {
  const sortedKeys = Object.keys(growthData).sort();
  return sortedKeys.map((key, idx) => {
    const cur = normalizeGrowthRow(growthData[key]);
    const prev =
      idx > 0 ? normalizeGrowthRow(growthData[sortedKeys[idx - 1]]) : null;
    const igDelta = prev !== null ? cur.IG - prev.IG : null;
    const ttDelta = prev !== null ? cur.TIKTOK - prev.TIKTOK : null;
    const xhDelta = prev !== null ? cur.XHS - prev.XHS : null;
    const totalCur = cur.IG + cur.TIKTOK + cur.XHS;
    const totalPrev = prev ? prev.IG + prev.TIKTOK + prev.XHS : null;
    return {
      key,
      isBase: idx === 0,
      igTotal: cur.IG,
      ...
      ttDelta,
      xhDelta,
      totalFollowers: totalCur,
      totalDelta: totalPrev !== null ? totalCur - totalPrev : null,
      notes: cur.notes,
    };
  });
}
function loadLocalBundle() {
  try {
    ...
    return {
      calendarData: parsed.calendarData ?? defaultCalendarData,
      growthData: parsed.growthData ?? defaultGrowthData,
    };
  } catch {
    return { calendarData: defaultCalendarData, growthData: defaultGrowthData };
  }
}
...
async function genPlatform({
  ...
  const fmtDesc = plat.formats
    .map((f) => `【${f.label}】字數上限${f.maxLen}字\n規則：${f.rules}`)
    .join("\n\n");
  const textPrompt = `你是天下第一好茶 BESTEA 的品牌社群小編，負責 ${
    plat.label
  } 平台。用戶會給你素材描述，你要改寫成符合品牌調性和平台風格的文案。
...
- Instagram：反問句或感嘆句，例：「一杯茶，憑什麼賣千元？」「喝過就回不去了。」
- 抖音：衝擊口語，例：「這茶也太扯了吧」「拜託別買 會上癮」「喝了十年茶 這款封神」
- 小紅書：數字+痛點，例：「喝了30款高山茶¸這款無限回購.ᐟ」「被問100次的茶¸今天公開」

${
  platformId === "instagram"
    ? `【IG 專屬規範】文青質感風，禁用「開賣」，改用「珍藏」「品味」等。Emoji 限用：🤍🫧☁️🍃✨🤎☕🌿🕊。符號：✦ ⟡ ˚ ⋆ ₊˚ ┊ ↳ ⌇ ─── 。全繁體中文。`
    : ""
}
${
  platformId === "tiktok"
    ? `【抖音 TikTok 專屬規範】超口語碎碎念接地氣。產品簡稱（梨山白茶→白茶，大禹嶺90K→90K，阿里山金萱→金萱）。短影片不用標點，用空格或Emoji斷句。全繁體中文。`
    : ""
}
${
  platformId === "xiaohongshu"
    ? `【小紅書專屬規範（非常重要）】必須全程使用繁體中文，絕對不可以出現任何簡體字。用¸代替所有逗號，用.ᐟ代替所有驚嘆號。Hashtag固定順序：#天下第一好茶 → #BESTEA → #besteatw → 主題相關標籤，全部用繁體中文。`
    : ""
}
...
${
  images.length > 0
    ? `【圖片素材】已附上${images.length}張圖片，仔細觀察每張圖中的產品、場景、氛圍，融入文案。`
    : ""
}
...
{
  "formats": [
    { "format_id": "${plat.formats[0].id}", "format_label": "${
    plat.formats[0].label
  }", "versions": [
      ...
      {item.title && (
        ...
        >
          ...
          <button
            onClick={copyTitle}
            style={{
              background: copiedTitle ? "#10B981" : "#ea580c",
              color: "#fff",
              border: "none",
              borderRadius: 7,
              padding: "5px 14px",
              ...
              {curFmt.versions.map((v, i) => (
                <button
                  ...
                  style={{
                    ...
                    border:
                      i === ver
                        ? `2px solid ${plat.accent}`
                        : "1.5px solid #e2e8f0",
                    background: i === ver ? `${plat.accent}10` : "#fff",
                    color: i === ver ? plat.accent : "#94a3b8",
                    fontSize: 12,
                    fontWeight: i === ver ? 800 : 500,
                    cursor: "pointer",
                    fontFamily: "inherit",
                  }}
                >
...
function SocialTab({ prefillBriefing = "", onClearPrefill }) {
  const [apiKey, setApiKey] = useState(
    () => localStorage.getItem("bestea_anthropic_key") || ""
  );
  ...
  const [history, setHistory] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("bestea_copy_history") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const saveKey = () => {
    localStorage.setItem("bestea_anthropic_key", apiKey);
    setShowKey(false);
  };
  const toggle = (id) =>
    setSelected((p) =>
  ...
  const generate = useCallback(async () => {
    if (!apiKey.trim()) {
      setError("請先輸入 Anthropic API Key");
      setShowKey(true);
      return;
    }
    if (!briefing.trim() && !images.length) {
      setError("請輸入活動描述或上傳圖片/影片");
      return;
    }
    if (!selected.length) {
      setError("請至少選擇一個平台");
    ...
    for (const pid of selected) {
      try {
        const res = await genPlatform({
          ...
          apiKey,
          stylePrompt:
            VIDEO_STYLES.find((s) => s.id === selectedStyle)?.prompt || "",
        });
        setResults((prev) => ({ ...prev, [pid]: res }));
      } catch (e) {
        setResults((prev) => ({ ...prev, [pid]: { error: e.message } }));
      }
      setLoadingMap((prev) => ({ ...prev, [pid]: false }));
      if (selected.indexOf(pid) < selected.length - 1)
    ...
    >
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input
              ...
              style={{ ...inputBase, flex: 1, borderColor: "#fcd34d" }}
            />
            <button
              onClick={saveKey}
              style={{
      ...
      {!showKey && (
        ...
        >
          <button
            ...
            style={{
              fontSize: 11,
              color: showHistory ? "#059669" : "#94a3b8",
              background: "none",
        ...
        >
          ...
          >
            ...
            <button
              onClick={() => {
                setHistory([]);
                localStorage.removeItem("bestea_copy_history");
              }}
              style={{
                fontSize: 10,
                color: "#dc2626",
        ...
        >
          ...
          {images.length === 0 && (
            ...
            >
              ...
              <div style={{ fontSize: 13, fontWeight: 600, color: "#999" }}>
                點擊上傳或拖曳（最多10張）
              </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  borderRadius: 12,
                  cursor: "pointer",
                  border: on ? `2px solid ${p.accent}` : "2px solid #EBEBEB",
                  background: on ? `${p.accent}06` : "#fff",
                  display: "flex",
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
          {VIDEO_STYLES.map((s) => {
            ...
            return (
              <button
                key={s.id}
                onClick={() => setSelectedStyle(on ? "" : s.id)}
                style={{
                  padding: "7px 12px",
                  borderRadius: 99,
                  border: on ? "2px solid #059669" : "1.5px solid #e2e8f0",
      ...
      <button
        ...
        style={{
          ...
          borderRadius: 12,
          border: "none",
          background: isLoading ? "#D4D4D4" : "#111",
          color: "#fff",
          fontSize: 14,
          fontWeight: 800,
        ...
        {selected.map((pid) => {
          ...
          if (res?.error)
            return (
              ...
              >
                ❌ {SOC_PLATFORMS.find((p) => p.id === pid)?.label}：{res.error}
              </div>
            );
          return (
            <PlatformCard
...
function GrowthCell({ val }) {
  if (val === null || val === undefined)
    return <span style={{ color: "#cbd5e1", fontSize: 12 }}>基準</span>;
  if (val > 0)
    return (
      <span
        style={{
...
export default function App() {
  ...
  const [calendarData, setCalendarData] = useState(
    () => loadLocalBundle().calendarData
  );
  const [growthData, setGrowthData] = useState(
    () => loadLocalBundle().growthData
  );
  ...
  useEffect(() => {
    growthDataRef.current = growthData;
  }, [growthData]);

  const cloudDocRef = useMemo(
    () => doc(db, CLOUD_DOC_PATH.collection, CLOUD_DOC_PATH.document),
    []
  );
  ...
  useEffect(() => {
    saveLocalBundle({ calendarData, growthData });
  }, [calendarData, growthData]);
  ...
  const writeCloudData = useCallback(
    async (bundle, reason = "manual") => {
  ...
  useEffect(() => {
    signInAnonymously(auth).catch((err) => {
      setSyncStatus(`雲端登入失敗：${formatError(err)}`);
      setAuthReady(true);
    });
    const unsub = onAuthStateChanged(auth, (user) => {
      setSyncStatus(user ? "已連線雲端" : "尚未登入雲端");
      setAuthReady(true);
    });
    return unsub;
  }, []);
      ...
      async (snapshot) => {
        try {
          ...
          if (data && (data.calendarData || data.growthData)) {
            const nc = data.calendarData ?? defaultCalendarData;
            const ng = data.growthData ?? defaultGrowthData;
            ...
            setSyncStatus("已同步最新雲端資料");
            setIsCloudReady(true);
          } else if (!initialSyncDoneRef.current) {
            const local = loadLocalBundle();
            await writeCloudData(local, "initial-create");
            skipNextUploadRef.current = true;
            hasLoadedCloudRef.current = true;
            initialSyncDoneRef.current = true;
            setCalendarData(local.calendarData);
            setGrowthData(local.growthData);
            setSyncStatus("已建立雲端排程表");
            setIsCloudReady(true);
          } else {
            hasLoadedCloudRef.current = true;
            setIsCloudReady(true);
          ...
          setSyncStatus(`初始化雲端失敗：${formatError(err)}`);
          hasLoadedCloudRef.current = true;
          setIsCloudReady(true);
        }
      },
      (err) => {
        setSyncStatus(`雲端同步失敗：${formatError(err)}`);
        hasLoadedCloudRef.current = true;
        setIsCloudReady(true);
      }
    );
        ...
        await writeCloudData(
          {
            ...
            growthData: growthDataRef.current,
          },
          "auto-save"
        );
        setSyncStatus("已自動儲存並同步");
      } catch (err) {
        setSyncStatus(`雲端儲存失敗：${formatError(err)}`);
      }
    }, 600);
    return () => {
      if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
  ...
  const forceSyncNow = useCallback(async () => {
    ...
    try {
      ...
      setSyncStatus("已手動同步成功");
    } catch (err) {
      setSyncStatus(`手動同步失敗：${formatError(err)}`);
    } finally {
  ...
  const reloadFromCloud = useCallback(async () => {
    if (!auth.currentUser) {
      setSyncStatus("尚未登入雲端，無法重新載入");
      return;
    }
    setIsManualSyncing(true);
    setSyncStatus("重新讀取雲端中...");
    try {
      const snap = await getDoc(cloudDocRef);
      const data = snap.data();
      if (data && (data.calendarData || data.growthData)) {
        skipNextUploadRef.current = true;
        setCalendarData(data.calendarData ?? defaultCalendarData);
        setGrowthData(data.growthData ?? defaultGrowthData);
        setLastSavedAt(data.updatedAt || "");
        setSyncStatus("已重新載入雲端資料");
      } else setSyncStatus("雲端目前沒有資料");
    } catch (err) {
      setSyncStatus(`重新讀取失敗：${formatError(err)}`);
    } finally {
      setIsManualSyncing(false);
    }
  }, [cloudDocRef]);
  ...
  const stats = useMemo(() => {
    ...
    Object.entries(calendarData).forEach(([date, raw]) => {
      ...
      if (y === currentYear && m - 1 === currentMonth) {
        const entries = getEntriesFromDay(raw);
        total += entries.length;
        posts += entries.filter((e) => e.hasPost).length;
        reels += entries.filter((e) => e.hasVideo).length;
        stories += entries.filter((e) => e.hasStory).length;
        scheduled += entries.filter((e) => e.scheduled).length;
  ...
  const monthEntries = useMemo(() => {
    ...
    Object.entries(calendarData).forEach(([date, raw]) => {
      const [y, m] = date.split("-").map(Number);
      if (y === currentYear && m - 1 === currentMonth)
        getEntriesFromDay(raw).forEach((item, index) =>
          rows.push({ date, index, ...item })
        );
    });
  ...
  const monthEntriesByPlatform = useMemo(
    () =>
      PLATFORM_OPTIONS.map((p) => ({
        ...p,
        items: monthEntries.filter((i) => i.platform === p.value),
      })),
    [monthEntries]
  );
  const growthRows = useMemo(() => computeGrowthRows(growthData), [growthData]);
  const currentGrowthRow = useMemo(
    () => growthRows.find((r) => r.key === currentMonthKey) || null,
    [growthRows, currentMonthKey]
  );
  ...
  const loadGrowthForm = useCallback(
    (monthKey) => {
      setGrowthMonth(monthKey);
      const raw = growthData[monthKey];
      if (raw) {
        const norm = normalizeGrowthRow(raw);
        setGrowthIG(norm.IG ? String(norm.IG) : "");
        setGrowthTT(norm.TIKTOK ? String(norm.TIKTOK) : "");
        setGrowthXH(norm.XHS ? String(norm.XHS) : "");
        setGrowthNotes(norm.notes || "");
      } else {
        setGrowthIG("");
        setGrowthTT("");
        setGrowthXH("");
  ...
  useEffect(() => {
    loadGrowthForm(currentMonthKey);
  }, [currentMonthKey, loadGrowthForm]);
  const clearGrowthForm = useCallback(() => {
    setGrowthIG("");
    setGrowthTT("");
    setGrowthXH("");
    setGrowthNotes("");
  ...
  const saveGrowth = async () => {
    ...
    const rowToSave = {
      IG: growthIG === "" ? 0 : Number(growthIG),
      TIKTOK: growthTT === "" ? 0 : Number(growthTT),
      XHS: growthXH === "" ? 0 : Number(growthXH),
      notes: growthNotes.trim(),
    };
    const nextGrowthData = { ...growthDataRef.current, [key]: rowToSave };
    setGrowthData(nextGrowthData);
    if (auth.currentUser) {
      try {
        setSyncStatus("粉絲數量儲存中...");
        await writeCloudData(
          { calendarData: calendarDataRef.current, growthData: nextGrowthData },
          "save-growth"
        );
        setSyncStatus("粉絲數量已儲存並同步");
      } catch (err) {
        setSyncStatus(`粉絲數量儲存失敗：${formatError(err)}`);
      }
    }
  };

  const deleteGrowthMonth = async (key) => {
    const next = { ...growthDataRef.current };
    delete next[key];
    setGrowthData(next);
    if (auth.currentUser) {
      try {
        await writeCloudData(
          { calendarData: calendarDataRef.current, growthData: next },
          "delete-growth"
        );
        setSyncStatus("已刪除並同步");
      } catch (err) {
        setSyncStatus(`刪除失敗：${formatError(err)}`);
      }
    }
  };
  ...
  const loadEntryToForm = useCallback((entry, dateStr) => {
    ...
    setEditDate(dateStr || "");
    setEditTitle(safe.title);
    setEditNotes(safe.notes);
    setEditCollaborators(safe.collaborators || "");
    setEditPlatform(safe.platform);
    setEditHasPost(safe.hasPost);
    setEditHasVideo(safe.hasVideo);
  ...
  const clearEditing = useCallback(() => {
    ...
    setEditPlatform("");
    setEditHasPost(false);
    setEditHasVideo(false);
    setEditHasStory(false);
  ...
  const openBlankEntryForDate = useCallback(
    (dateStr) => {
      ...
      setSelectedDateEntries([normalizeEntry()]);
      setEditingIndex(0);
      setEditDate(dateStr);
      clearEditing();
    },
  ...
  const handleDateClick = useCallback(
    (dateStr, targetIndex = 0) => {
      const entries = getEntriesFromDay(calendarData[dateStr]);
      const safeEntries = entries.length ? entries : [normalizeEntry()];
      const safeIndex = Math.min(targetIndex, safeEntries.length - 1);
      setSelectedDate(dateStr);
      setSelectedDateEntries(safeEntries);
      setEditingIndex(safeIndex);
      loadEntryToForm(safeEntries[safeIndex], dateStr);
    },
    [calendarData, loadEntryToForm]
  );
  ...
  const addNewScheduleItem = () => {
    ...
    setSelectedDateEntries((prev) => {
      ...
      next.push(normalizeEntry());
      return next;
    });
    setEditingIndex((p) => p + 1);
    clearEditing();
  };
  const removeCurrentEditingItemOnly = () => {
    setSelectedDateEntries((prev) => {
      if (!prev.length) return prev;
      const next = prev.filter((_, i) => i !== editingIndex);
      if (next.length === 0) {
        setEditingIndex(0);
        clearEditing();
        return [normalizeEntry()];
      }
      const ni = Math.max(0, editingIndex - 1);
      setEditingIndex(ni);
      loadEntryToForm(next[ni], editDate);
      return next;
    });
  };
  ...
  const saveEntry = async () => {
    ...
    const draft = normalizeEntry({
      ...(selectedDateEntries[editingIndex] ?? normalizeEntry()),
      ...getFormValues(),
    });
    const nextDayEntries = [...selectedDateEntries];
    nextDayEntries[editingIndex] = draft;
    const packed = packEntriesForSave(nextDayEntries);
    const next = { ...calendarDataRef.current };
    if (selectedDate && selectedDate !== targetDate) {
      const oldEntries = getEntriesFromDay(next[selectedDate]).filter(
        (_, i) => i !== editingIndex
      );
      const oldPacked = packEntriesForSave(oldEntries);
      if (!oldPacked) delete next[selectedDate];
      else next[selectedDate] = oldPacked;
    }
    if (!packed) delete next[targetDate];
    else next[targetDate] = packed;
    ...
    if (auth.currentUser) {
      try {
        setSyncStatus("儲存中...");
        await writeCloudData(
          { calendarData: next, growthData: growthDataRef.current },
          "save-entry"
        );
        setSyncStatus("已儲存並同步");
      } catch (err) {
        setSyncStatus(`儲存同步失敗：${formatError(err)}`);
      }
    }
  };
  ...
  const deleteEntry = async () => {
    ...
    if (auth.currentUser) {
      try {
        ...
        setSyncStatus(`刪除同步失敗：${formatError(err)}`);
      }
    }
  };
  ...
  const exportCSV = useCallback(() => {
    const rows = [
      ...
      ...Object.entries(calendarDataRef.current)
        .sort((a, b) => a[0].localeCompare(b[0]))
        .flatMap(([date, raw]) =>
          getEntriesFromDay(raw).map((item, idx) => [
            date,
            idx + 1,
    ...
    downloadCSV(
      `content-calendar-${currentYear}-${String(currentMonth + 1).padStart(
        2,
        "0"
      )}.csv`,
      rows
  ...
  const exportGrowthCSV = useCallback(() => {
    const rows = [
      ...
      ...growthRows.map((r) => [
        r.key,
        r.igTotal,
        r.igDelta !== null ? r.igDelta : "基準",
        r.ttTotal,
        r.ttDelta !== null ? r.ttDelta : "基準",
        r.xhTotal,
        r.xhDelta !== null ? r.xhDelta : "基準",
        r.totalFollowers,
        r.totalDelta !== null ? r.totalDelta : "基準",
        r.notes,
      ]),
    ];
        ...
        <div className="container">
          <div className="header">
            <div className="left-head">
              ...
              <h1 className="title">
                Bestea <span className="accent">多平台</span> 排程表
              </h1>
              <div className="tabs">
                {TAB_OPTIONS.map((tab) => {
                  const Icon = tab.icon;
                ...
                <>
                      ...
                      >
                        {monthNames.map((name, idx) => (
                          <option
                            key={idx}
                            value={idx}
                            disabled={currentYear === 2026 && idx < 2}
                          >
                            {name}
                          </option>
                        ))}
                  ...
                  <div className="head-actions">
                    <button
                      ...
                      onClick={
                        mainTab === "planner" ? exportCSV : exportGrowthCSV
                      }
                    >
                      <Download size={14} />
                      {mainTab === "planner"
                    ...
                    <div className="sync-chip compact">
                      {!authReady || !isCloudReady ? (
            ...
            {mainTab === "planner" && (
              <div className="stats">
                <div className="stat-card">
                  <div className="stat-label">貼文</div>
                ...
                <div className="stat-card">
                  <div className="stat-label">限時動態</div>
                  <div className="stat-value">
                ...
                <div className="stat-card">
                  <div className="stat-label">待發布</div>
                  <div className="stat-value">
                    <ClipboardList size={15} color="#475569" />
                    {stats.pending}
                  </div>
                </div>
              </div>
              ...
              (currentGrowthRow ? (
                <div className="growth-stats-header">
                  <div className="stat-card">
                    ...
                    <div className="stat-value">
                      ...
                      {formatNumber(currentGrowthRow.igTotal)}
                    </div>
                  </div>
                  <div className="stat-card">
                  ...
                  <div className="stat-card">
                    <div className="stat-label">本月成長</div>
                    <div
                      className="stat-value"
                      style={{
                        fontSize: currentGrowthRow.isBase ? 13 : 22,
                        color: currentGrowthRow.isBase
                          ? "#7c3aed"
                          : (currentGrowthRow.totalDelta || 0) >= 0
                          ? "#059669"
                          : "#dc2626",
                      }}
                    >
                      {currentGrowthRow.isBase ? (
                        "基準月"
                      ) : (
                        <>
                          {(currentGrowthRow.totalDelta || 0) >= 0 ? (
                          ...
                          {formatSigned(currentGrowthRow.totalDelta)}
                        </>
                      )}
                    </div>
                  </div>
                ...
                <div className="growth-stats-header">
                  <div className="stat-card" style={{ gridColumn: "1/-1" }}>
                    <div className="stat-label">尚無粉絲資料</div>
                    <div
          ...
          <div style={{ display: mainTab === "social" ? "block" : "none" }}>
            <SocialTab
              prefillBriefing={socialPrefill}
              onClearPrefill={() => setSocialPrefill("")}
            />
          </div>
          ...
          {mainTab === "planner" && (
            <div
              className={`content-layout ${sidebarCollapsed ? "expanded" : ""}`}
            >
              <div>
                <div className="calendar-shell">
                  ...
                  <div className="calendar-grid">
                    {Array.from({ length: firstDayOfMonth }).map((_, i) => (
                      <div key={`es-${i}`} className="empty-cell" />
                    ))}
                        ...
                        >
                          <div className="day-top">
                            ...
                            <div className="day-icons">
                              {summary.hasPost && (
                                <Image size={10} color="#7c3aed" />
                              )}
                              {summary.hasVideo && (
                                <Video size={10} color="#059669" />
                              )}
                              {summary.hasStory && (
                                <Smartphone size={10} color="#f59e0b" />
                              )}
                              {summary.total > 1 && (
                              ...
                              {summary.scheduled && !summary.published && (
                                <span className="scheduled-check">排</span>
                              )}
                              {summary.published && (
                          ...
                          {entries.length ? (
                            <div className="day-items">
                              {entries.slice(0, 2).map((item, idx) => {
                                const meta = getPlatformMeta(item.platform);
                                const PIcon = meta ? meta.icon : null;
                                return (
                                  <div
                                    key={item.id || idx}
                                    className={`day-title ${
                                  ...
                                  >
                                    {PIcon && (
                                      <div className="platform-mini">
                                        <PIcon size={9} />
                                        {PLATFORM_LABEL_MAP[item.platform]}
                                      </div>
                                    )}
                                    {item.title || "未命名內容"}
                                  </div>
                    ...
                    {Array.from({ length: trailingEmptyCells }).map((_, i) => (
                      <div key={`ee-${i}`} className="empty-cell" />
                    ))}
                  </div>
                </div>
                <div className="footer">
                  ...
                  <div className="legend">
                    {[
                      { color: "#7c3aed", label: "📷 貼文" },
                      { color: "#10b981", label: "🎥 REELS" },
                      { color: "#f59e0b", label: "📱 限時動態" },
                      { color: "#2563eb", label: "排 已排程" },
                      { color: "#dc2626", label: "✓ 已發布" },
                    ].map(({ color, label }) => (
                  ...
                  <div className="sidebar-tools">
                    <div>
                      ...
                      <div className="sidebar-subtitle">
                        {currentYear} 年 {monthNames[currentMonth]} ｜共{" "}
                        {stats.total} 筆
                      </div>
                    </div>
                    <div className="sidebar-tool-actions">
                      <button
                        className="mini-btn"
                        type="button"
                        onClick={() => openBlankEntryForDate(today)}
                      >
                        <Plus size={13} />
                        新增
                      </button>
                          ...
                          <div key={group.value} className="platform-section">
                            ...
                            <div className="entry-list">
                              {group.items.length === 0 ? (
                                <div className="empty-state">尚無內容</div>
                              ) : (
                                group.items.map((item) => (
                                  <div
                                    ...
                                    onClick={() =>
                                      handleDateClick(item.date, item.index)
                                    }
                                  >
              ...
              <div className="growth-form-shell">
                <div className="growth-form-body">
                  ...
                  <div className="section-subtitle">
                    每月輸入當下的粉絲總數，系統自動與上月比較並計算成長幅度。
                  </div>
                  <div className="info-box">
                    <Info size={14} style={{ marginTop: 1, flexShrink: 0 }} />
                    <span>
                      請直接輸入<strong>目前各平台粉絲總數</strong>
                      。系統會自動與上個月比較，算出成長幅度。
                      <strong>2026年3月</strong>為基準月，不計算成長。
                    </span>
                  </div>
                    ...
                    <div className="field-group" style={{ marginBottom: 0 }}>
                      ...
                      <div className="select-wrap" style={{ display: "block" }}>
                        <select
                          ...
                          value={growthMonth}
                          onChange={(e) => loadGrowthForm(e.target.value)}
                        >
                          {(() => {
                            const opts = [];
                  ...
                  <div className="three-cols">
                    <div className="field-group">
                      ...
                      <input
                        ...
                        min="0"
                        value={growthIG}
                        onChange={(e) => setGrowthIG(e.target.value)}
                      />
                    </div>
                    <div className="field-group">
                      <label
                      ...
                      <input
                        ...
                        min="0"
                        value={growthTT}
                        onChange={(e) => setGrowthTT(e.target.value)}
                      />
                    </div>
                    <div className="field-group">
                      <label
                      ...
                      <input
                        ...
                        min="0"
                        value={growthXH}
                        onChange={(e) => setGrowthXH(e.target.value)}
                      />
                    </div>
                  </div>
                    ...
                    <tbody>
                      {growthRows.map((row) => {
                        ...
                        return (
                          ...
                          >
                            <td>
                              <span className="month-label">{row.key}</span>
                              {row.isBase && (
                                <span className="base-badge">基準</span>
                              )}
                              {isCurrent && (
                                <span className="current-badge">本月</span>
                              )}
                            </td>
                            <td className="num-cell">
                              {formatNumber(row.igTotal)}
                            </td>
                            <td>
                              <GrowthCell
                                val={row.isBase ? null : row.igDelta}
                              />
                            </td>
                            <td className="num-cell">
                              {formatNumber(row.ttTotal)}
                            </td>
                            <td>
                              <GrowthCell
                                val={row.isBase ? null : row.ttDelta}
                              />
                            </td>
                            <td className="num-cell">
                              {formatNumber(row.xhTotal)}
                            </td>
                            <td>
                              <GrowthCell
                                val={row.isBase ? null : row.xhDelta}
                              />
                            </td>
                            <td
                              className="num-cell"
                            ...
                            <td>
                              <GrowthCell
                                val={row.isBase ? null : row.totalDelta}
                              />
                            </td>
                            ...
                            <td style={{ whiteSpace: "nowrap" }}>
                              <button
                                ...
                                onClick={() => loadGrowthForm(row.key)}
                              >
                                編輯
                              </button>
                              <button
                                className="del-row-btn"
                                onClick={() => deleteGrowthMonth(row.key)}
                                title="刪除此月"
                              >
                                <X size={11} />
                              </button>
          ...
          <div className="overlay">
            <div className="modal">
              <div className="modal-head">
                <div className="modal-head-title">編輯排程</div>
              ...
              <div className="modal-body">
                <div className="schedule-tabs">
                  {selectedDateEntries.map((entry, idx) => {
                    ...
                    const PIcon = meta ? meta.icon : Calendar;
                    ...
                    return (
                      <button
                        ...
                        className={`schedule-tab ${
                          editingIndex === idx ? "active" : ""
                        }`}
                        onClick={() => switchEditingItem(idx)}
                        style={{
                      ...
                      >
                        <PIcon size={12} style={{ color }} />
                        {idx + 1}. {entry.title || "未命名"}
                      </button>
                    );
                  })}
                ...
                <div className="two-cols">
                  ...
                  <div className="field-group">
                    <label className="field-label">協作者</label>
                    <input
                      className="text-input"
                      type="text"
                      ...
                      return (
                        <button
                          ...
                          className={`platform-btn ${
                            editPlatform === p.value ? `active-${p.value}` : ""
                          }`}
                          onClick={() =>
                            setEditPlatform(
                              editPlatform === p.value ? "" : p.value
                            )
                          }
                        >
                          <Icon size={14} />
                  ...
                  <div className="post-type-group">
                    <button
                      ...
                      className={`post-type-btn ${
                        editHasPost ? "active-post" : ""
                      }`}
                      onClick={() => setEditHasPost((v) => !v)}
                    >
                      <Image size={20} />
                      貼文
                    </button>
                    <button
                      type="button"
                      className={`post-type-btn ${
                        editHasVideo ? "active-video" : ""
                      }`}
                      onClick={() => setEditHasVideo((v) => !v)}
                    >
                      <Video size={20} />
                      REELS
                    </button>
                    <button
                      type="button"
                      className={`post-type-btn ${
                        editHasStory ? "active-story" : ""
                      }`}
                      onClick={() => setEditHasStory((v) => !v)}
                    >
                      <Smartphone size={20} />
                      限時動態
                    </button>
                  ...
                  <div className="status-group">
                    <button
                      ...
                      className={`status-btn ${
                        editScheduled ? "active-scheduled" : ""
                      }`}
                      onClick={() => setEditScheduled((v) => !v)}
                    >
                      <Calendar size={15} />
                      已排程
                    </button>
                    <button
                      type="button"
                      className={`status-btn ${
                        editPublished ? "active-published" : ""
                      }`}
                ...
                <div className="modal-actions">
                  ...
                  <button
                    className="minor-btn"
                    onClick={removeCurrentEditingItemOnly}
                    type="button"
                  >
                    <X size={13} />
                    刪除這一筆
