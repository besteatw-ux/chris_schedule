...
const makeIcon = (symbol) => function Icon({ size = 16, color, className = "", style = {}, ...props }) {
  return (
    <span
      className={className}
      style={{
...
const SOC_PLATFORMS = [
  {
    ...
    formats: [
      { id: "reels", label: "Reels 短影片", maxLen: 80, rules: "建議80字左右，依內容需要調整，2-3句。質感生活切入，有畫面感，像詩一樣有停頓節奏。開頭要有畫面感如「一包茶，一個清晨」。口語但文青，配音旁白感。如果是促銷/新品/送禮類，結尾加 ☞ 主頁有更多；日常/美感/故事類讓文案自然結尾。【IG Hashtag 2026規則】Instagram 限制最多5個標籤。固定放：#天下第一好茶 #BESTEA，再加3個精準中型標籤(5萬-50萬貼文量)，依該篇茶葉品種/生活情境選擇。絕對不用百萬級大標籤。注意：title欄位留空，IG不需要標題。" },
    ],
  },
  {
    id: "tiktok", label: "抖音 / TikTok", icon: "🎵", accent: "#111",
    formats: [
      { id: "video", label: "短影片", maxLen: 70, rules: "建議70字左右，依內容需要調整，2-4句。不用標點符號，用空格或Emoji斷句。【受眾語調】國際版TikTok，受眾以台灣、東南亞、歐美華人為主，18-34歲Gen Z。語氣輕鬆自然、口語生動，避免純台灣用語如「扯」「超ㄅ」「靠邀」，改用通用口語如「原來如此」「真的沒想到」「值得一試」。台灣品牌特色保留。如果是促銷/新品/送禮類，結尾加 👉🏻 主頁看更多；日常/美感/故事類讓文案自然結尾。【抖音 Hashtag 2026規則】3-5個標籤效果最好。固定放：#天下第一好茶 #BESTEA，再加1-3個精準主題標籤，依影片內容選擇。注意：title欄位留空，抖音不需要標題。" },
    ],
  },
  {
          ...
          <div className="header">
            ...
            {mainTab === "growth" && (currentGrowthRow ? (
              <div className="growth-stats-header">
                ...
                <div className="stat-card"><div className="stat-label">TikTok</div><div className="stat-value"><Music size={15} />{formatNumber(currentGrowthRow.ttTotal)}</div></div>
                <div className="stat-card"><div className="stat-label">小紅書</div><div className="stat-value"><BookOpen size={15} color="#fe2c55" />{formatNumber(currentGrowthRow.xhTotal)}</div></div>
            ...
            <div className="growth-layout">
              <div className="growth-form-shell">
                <div className="growth-form-body">
                  <div className="section-title"><BarChart size={17} />登錄粉絲數</div>
                  ...
                  <div className="three-cols">
                    ...
                    <div className="field-group"><label className="field-label" style={{ display: "flex", alignItems: "center", gap: 4 }}><Music size={10} />TikTok 總粉絲數</label><input className="number-input" type="number" min="0" value={growthTT} onChange={e => setGrowthTT(e.target.value)} /></div>
                    <div className="field-group"><label className="field-label" style={{ display: "flex", alignItems: "center", gap: 4 }}><BookOpen size={10} color="#fe2c55" />小紅書 總粉絲數</label><input className="number-input" type="number" min="0" value={growthXH} onChange={e => setGrowthXH(e.target.value)} /></div>
                  </div>
              ...
              {growthRows.length > 0 && (
                <div className="growth-table-shell">
                  <table className="growth-table">
                    <thead><tr><th>月份</th><th><Instagram size={10} style={{ verticalAlign: "middle", marginRight: 3 }} />IG 總粉絲</th><th>IG 成長</th><th><Music size={10} style={{ verticalAlign: "middle", marginRight: 3 }} />TikTok 總粉絲</th><th>TikTok 成長</th><th><BookOpen size={10} style={{ verticalAlign: "middle", marginRight: 3 }} />小紅書總粉絲</th><th>小紅書成長</th><th>三平台總計</th><th>總成長</th><th>備註</th><th></th></tr></thead>
                    <tbody>
                      {growthRows.map(row => {
